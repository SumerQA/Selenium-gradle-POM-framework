...
public class events {
	...
	public void Setvalue_HiddenInputFieldJS(String path, String val) {
		((JavascriptExecutor) driver).executeScript(path+val+";");
	}
	...
	public void Table_event_With_Pagination(String compTest, String ePath,String ePath1) throws IOException { // Will get the table cells data and click on perticular cell
		WebElement element = driver.findElement(By.xpath(c.getElementName(ePath)));
		List<WebElement> rows = element.findElements(By.tagName("tr"));
		String totalRecords=driver.findElement(By.xpath(c.getElementName(ePath1)+"/div[3]/span")).getText();
		int totalRecord=extractMaximum(totalRecords);
		if (totalRecord>30) {totalRecord=(totalRecord + 30 -1)/30;}
		else {totalRecord=1;}
		labrow:  for(int plist = 1; plist<= totalRecord; plist++)
		{
			int rowno=0;
			...
			if(plist<totalRecord)
			{
				driver.findElement(By.xpath(c.getElementName(ePath1)+"/div[3]/a[3]/span")).click();
	...
	public void CheckItem_AfterDeleting_In_Table_event(String compTest,	String ePath) throws IOException { // Will get the table cells data
		...
		for (int rownum = 0; rownum < rows.size(); rownum++) {
			...
			for (int colnum = 0; colnum < columns.size(); colnum++) {
				// System.out.println(columns.get(colnum).getText());
				if (columns.get(colnum).getText().equals(compTest)) {
					throw new ElementNotInteractableException(compTest+ "Exists in the table");
				} else {
					continue;
				}
			}
	...
	public void CheckItem_AfterAdding_In_Table_event(String compTest,String ePath) throws IOException { // Will get the table cells data
												// and click on particular cell
		WebElement element = driver.findElement(By.xpath(c.getElementName(ePath)));
		List<WebElement> rows = element.findElements(By.tagName("tr"));
		for (int rownum = 0; rownum < rows.size(); rownum++) {
			...
			for (int colnum = 0; colnum < columns.size(); colnum++) {
				// System.out.println(columns.get(colnum).getText());
				if (!columns.get(colnum).getText().equals(compTest)) {
					throw new ElementNotInteractableException(compTest+ "Not Exists,Still not added in the table");
				}
			}
		}
	}
